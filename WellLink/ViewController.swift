...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        syncButton.setTitle("Sync Health Data Now", for: .normal)
        syncButton.titleLabel?.font = .systemFont(ofSize: 20, weight: .bold)
...
// ── Singleton manager — handles all sync logic ────────────────────────────────
class HealthSyncManager {
    ...
    let backendURL  = "http://subcubical-cami-undeferential.ngrok-free.dev/api/health"
    ...
    func scheduleWeeklySync() {
        ...
        UNUserNotificationCenter.current().add(request) { error in
            if let error = error {
                ...
                print("Weekly sync notification scheduled for every Monday 8am")
            }
        }
    }
    ...
    //   com.yourname.healthsync.weeklysync
    func scheduleBackgroundTask() {
        let request = BGAppRefreshTaskRequest(
            identifier: "com.yourname.healthsync.weeklysync"
        )
    ...
    private func fetchAndSend(daysBack: Int, completion: @escaping (Bool) -> Void) {
        ...
        var result: [String: Any] = ["userId": "user_123"]
    ...
    private func send(payload: [String: Any], completion: @escaping (Bool) -> Void) {
        ...
        URLSession.shared.dataTask(with: req) { _, response, error in
            ...
            print(success ? "Sync sent successfully" : "Sync failed: \(error?.localizedDescription ?? "")")
            completion(success)
        }.resume()
    }
}
